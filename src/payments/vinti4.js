...
module.exports = {
  ...
  generatePaymentForm: (posID, posAutCode, fileName, responseUrl, storeId, orderId, orderTotal) => {
    ...
    const postURL = `${vinti4_url}?FingerPrint=${encodeURIComponent(formData.fingerprint)}&TimeStamp=${encodeURIComponent(formData.timeStamp)}&FingerPrintVersion=${encodeURIComponent(formData.fingerprintversion)}`;

    // Create form to POST to Vinti4
  ...
  processWebhook: async (posAutCode, data) => {
